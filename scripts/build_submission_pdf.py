...
def architecture():
    ...
    box(d, 190, 38, 120, 42, "File Storage", "Local v1 adapter", colors.HexColor("#FFF7E6"))
    box(d, 350, 38, 120, 42, "Notifications", "In-app inbox", colors.HexColor("#FFF0F0"))
...
story += [Paragraph("3. API and Interaction Overview",styles["H1x"]), decision_table([
    ...
    ("Notifications","External email dependency","Deduplicated in-app alerts keep the local v1 self-contained and require no domain or provider account."),
...
]), Spacer(1,8*mm), Paragraph("Implemented routes",styles["H2x"]), Paragraph("<b>/login</b> credentials and session creation; <b>/</b> live dashboard; <b>/tasks</b> Kanban/list/calendar, workflow, reassignment, dependencies, comments, photos and CSV; <b>/rcas</b> structured review and reviewer reassignment; <b>/notifications</b> inbox; <b>/reports</b> live analytics; <b>/settings</b> membership and roles.",styles["Bodyx"]), PageBreak()]
...
story += [Paragraph("5. Delivery Status and Evidence",styles["H1x"]), Paragraph("The repository includes two PostgreSQL migrations, deterministic seed data, protected server-rendered screens, adapter interfaces, architecture documentation, and automated tests for the highest-risk rules.",styles["Bodyx"]), decision_table([
    ...
    ("Domain tests","17 passing","Sessions, transitions, mentions, notification policy, RCA decisions, CSV filtering, and warnings."),
    ("Security","Implemented","Scrypt password hashes, signed HTTP-only cookies, server permission checks, no committed secrets."),
    ("Known limits","Documented","External email is deferred; production object storage, identity provider, and deployment remain open.")
